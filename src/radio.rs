...
impl Radio {
    ...
    pub fn new(packet_length_bytes: u16, on_packet_received: fn()) -> RailResult<Self> {
    ...
    fn init(p_rail_config: sl_rail_config_t) -> RailResult<sl_rail_handle_t> {
        ...
        unsafe {
            ...
            sl_rail_config_cal(p_rail_handle, SL_RAIL_CAL_ALL).into_rail_result()?;
            ...
            // sl_rail_set_tx_power_dbm(p_rail_handle, 20 /* 20 dBm */).into_rail_result()?;
            // we can't use the above because its a macro that doesn't get auto-generated by bindgen
            RAIL_SetTxPowerDbm(p_rail_handle, 20 /* 20 dBm */).into_rail_result()?;
            ...
            sl_rail_set_rx_transitions(p_rail_handle, &p_state_transitions).into_rail_result()?;
            sl_rail_set_tx_transitions(p_rail_handle, &p_state_transitions).into_rail_result()?;
    ...
    pub fn send_packet(&self, packet: &[u8]) -> RailResult<()> {
        unsafe {
            ...
            if bytes_written != self.packet_length_bytes {
